...
update-pricing:
	@if [ -d .venv ]; then \
		. .venv/bin/activate && python scripts/openrouter/update_model_pricing.py update-pricing $(ARGS); \
	elif [ -d venv ]; then \
		. venv/bin/activate && python scripts/openrouter/update_model_pricing.py update-pricing $(ARGS); \
	else \
		python3 scripts/openrouter/update_model_pricing.py update-pricing $(ARGS); \
	fi
