...
PLOT_TITLE = "LLM Ensemble vs Human Labels"
...
def load_evaluation_data(run_name: str):
    """Load ground truth and predictions from aggregate run.

    Args:
        run_name: Name of the aggregate run

    Returns:
        Tuple of (ground_truth, predictions) as lists of integers
    """
    reader = IOAdapterFactory.create_reader("db_aggregate_to_json")
    ...
    predictions = [int(pred) if pred is not None else -1 for pred in eval_data.predictions]
...
def plot_confusion_matrix_figure(
    ...
    cm = confusion_matrix(gt_valid, pred_valid, labels=[0, 1, 2, 3], normalize=normalize)
...
def print_confusion_analysis(ground_truth: list, predictions: list):
    ...
    for i, label in enumerate(["Irrelevant", "Relevant", "Highly Relevant", "Perfectly Relevant"]):
        row_sum = cm[i].sum()
        if row_sum > 0:
    ...
    typer.echo(f"\nBias Analysis:")
    typer.echo(f"  Over-predicting (LLM > Human): {over_predict:,} ({over_predict/len(gt_valid):.1%})")
    typer.echo(f"  Under-predicting (LLM < Human): {under_predict:,} ({under_predict/len(gt_valid):.1%})")
    ...
    typer.echo(f"\nError Distance:")
    typer.echo(f"  Adjacent errors (off by 1): {adjacent_errors:,} ({adjacent_errors/len(gt_valid):.1%})")
    typer.echo(f"  Distant errors (off by 2+): {distant_errors:,} ({distant_errors/len(gt_valid):.1%})")


@app.command()
def main(
    run_name: str = typer.Option(RUN_NAME, "--run-name", "-r", help="Aggregate run name"),
    normalize: str = typer.Option(NORMALIZE, "--normalize", "-n", help="Normalization: none, true, pred, all"),
):
    ...
    typer.echo(f"Loading data from aggregate run: {run_name}")

    # Load data
    ground_truth, predictions = load_evaluation_data(run_name)
