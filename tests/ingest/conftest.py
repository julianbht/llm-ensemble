"""Pytest fixtures for ingest CLI tests.

This module provides shared test fixtures that are automatically discovered
by pytest and available to all test files in the tests/ingest/ directory.
"""
...
def sample_dataset() -> NormalizedDataset:
    ...
    the ingest pipeline without requiring real data files.
...
def sample_dataset_five() -> NormalizedDataset:
    """Create a test dataset with five samples for limit tests.
